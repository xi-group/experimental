...
resource "aws_internet_gateway" "foiye_igw" {
	vpc_id	= aws_vpc.foiye.id
...
resource "aws_default_route_table" "default_rt" {
	default_route_table_id	= aws_vpc.foiye.main_route_table_id
...
resource "aws_route" "route_igw" {
	...
	gateway_id		= aws_internet_gateway.foiye_igw.id
...
resource "aws_nat_gateway" "foiye_natgw" {
...
resource "aws_route" "private_nat_gateway" {
	route_table_id		= aws_route_table.private_rt.id
	destination_cidr_block	= "0.0.0.0/0"
	nat_gateway_id		= aws_nat_gateway.foiye_natgw.id
}
